...
function enforceRubricAlignment(questions, payload) {`,
  ...
  replaceOnce(
    "server/assessment-service.js",
    "Buat rubric khusus untuk setiap soal berdasarkan pertanyaan yang dibuat dan learning_outcome. Rubrik harus berisi 3-4 indikator yang dapat diamati, lengkap dengan bobot total 100%, dan hanya menilai isi yang benar-benar diminta oleh pertanyaan serta selaras dengan learning outcome.",
  ...
  const oldRequestRepair = `async function requestRepair(mode, result) {
  ...
  const streamingRequestRepair = fs.readFileSync(path.join(ROOT, "scripts", "pedagogical-gate-request-repair-streaming.txt"), "utf8");
  replaceOnce("src/js/pedagogical-gate.js", oldRequestRepair, streamingRequestRepair, "streaming repair request");
