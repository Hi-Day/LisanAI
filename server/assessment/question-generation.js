...
const SINGLE_SUBSTANCE_RULES = [
  ...
  "DILARANG pertanyaan bertingkat yang meminta beberapa hal sekaligus dalam satu soal.",
  "Setiap soal cukup SATU kalimat tanya dengan SATU tanda tanya; hindari kata penghubung tugas ganda.",
  "Jika konteks diperlukan, tulis konteks sebagai kalimat informasi di awal soal, lalu akhiri SATU pertanyaan/instruksi tunggal yang bisa dijawab lisan.",
].join(" ");
...
const ORAL_SCENARIO_RULES = [
  ...
  "Tetap patuhi prinsip SATU substansi dan satu tujuan.",
].join(" ");
...
const SCHEMA = 'Format: {"questions":[{"prompt":"...","focus":"...","outcome":"...","rubric":"indikator 40%\\nindikator 35%\\nindikator 25%","ideal":"...","criteria":["nama_kriteria1","nama_kriteria2"]}]}. rubric WAJIB khusus untuk setiap soal. Jumlah questions harus sesuai jumlah_soal.';
...
async function generateQuestions(payload) {
  ...
  return normalizeGenerated(result.questions.slice(0, count), payload);
}
...
async function streamGenerateQuestions(payload, onChunk) {
  ...
  const parsed = alignment.parseJson(content);
  if (!Array.isArray(parsed.questions)) throw new Error("Model tidak mengembalikan daftar soal");
  return normalizeGenerated(parsed.questions.slice(0, count), payload);
}

function normalizeGenerated(rawQuestions, payload) {
  return rawQuestions.map(normalizeQuestion(payload));
}

module.exports = { generateQuestions, streamGenerateQuestions, buildMessages, normalizeQuestion, SINGLE_SUBSTANCE_RULES, ORAL_SCENARIO_RULES };