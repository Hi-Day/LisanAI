...
async function handleStreamingAction(res, auth, action, payload) {
  ...
  try {
    ...
    else if (action === "evaluate") { const { evaluateWithHarness } = require("../harness/harness-evaluator"); const evaluation = await evaluateWithHarness({ ...payload, auth, onProgress: (text) => { if (res.writableEnded === false) writeSse(res, { type: "chunk", text }); } }); writeSse(res, { type: "chunk", text: "Evaluasi selesai." }); writeSse(res, { type: "result", data: { evaluation, harness: true } }); }
    else if (action === "generate-probing") writeSse(res, { type: "result", data: { probing: await buildAdaptiveProbe(payload, auth, onChunk) } });
...
module.exports = async (req, res) => {
  try {
    ...
    let viaApiKey = false;
    if (!auth) { const apiAuth = await authenticateApiKey(req); if (apiAuth) { auth = { tenant: { id: apiAuth.tenantId, name: "API", plan: "api" }, user: { id: `apikey:${apiAuth.keyId}`, tenantId: apiAuth.tenantId, name: "API Key", role: "admin" } }; viaApiKey = true; } }
    ...
    if (stream === true) {
      if (action === "evaluate" && auth.user.role === "student") { try { await assertCanSubmitAssessment(auth.tenant.id, auth.user.id, payload.assessment.id); } catch (e) { return sendJson(res, e.status || 403, { error: e.message }); } }
      if (action !== "evaluate" && action !== "generate-probing" && !["admin", "teacher"].includes(auth.user.role)) return sendJson(res, 403, { error: "Forbidden" });
      return handleStreamingAction(res, auth, action, payload);
    }
    if (action === "evaluate") {
      if (auth.user.role === "student") { try { await assertCanSubmitAssessment(auth.tenant.id, auth.user.id, payload.assessment.id); } catch (e) { return sendJson(res, e.status || 403, { error: e.message }); } }
      const { evaluateWithHarness } = require("../harness/harness-evaluator");
    ...
    if (action === "recommend-learning-outcomes" || action === "recommend-assessment-config") { const outcomes = await recommendLearningOutcomes({ ...payload, count: 3 }); return sendJson(res, 200, { recommendation: recommendationFromOutcomes(outcomes), outcomes, count: outcomes.length, model: process.env.OPENROUTER_MODEL }); }
