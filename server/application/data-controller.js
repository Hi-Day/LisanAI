const {
  deleteAssessment, getState, saveAssessment,
  updateAssessment,
...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === "GET") {
      ...
      if (action === "state") { await ensureShowcaseDemo(); return sendJson(res, 200, await getState(auth)); }
    ...
    if (action === "remove-demo-data") {
      if (!isTeacherOrAdmin) return sendJson(res, 403, { error: "Forbidden" });
      const { removeDemoData } = require("../seed-demo"); return sendJson(res, 200, await removeDemoData(auth.tenant.id));
    }
    ...
    if (action === "save-submission") {
      if (isStudent) {
        ...
        try { const { broadcast } = require("../../api/notifications"); broadcast({ type: "submission", title: "Penilaian baru", message: `${auth.user.name} mengumpulkan "${payload.assessmentTitle || "penilaian"}"`, assessmentId: payload.assessmentId, tenantId: auth.tenant.id }); } catch { }
      ...
      try {
        ...
        if (runId && payload.finalScore !== undefined && payload.finalScore !== null) await recordTeacherScoreChange({ runId, finalScore: payload.finalScore, tenantId: auth.tenant.id, reviewerId: auth.user.id, reviewNote: (payload.questionScores || []).find((qs) => qs && qs.complaint && qs.complaint.status === "resolved") ? "Komplain siswa diterima." : "" });
    ...
    if (action === "submit-complaint") {
      ...
      return sendJson(res, 200, { submission: await submissionService.saveComplaint(auth, submissionId, questionIndex, reason) });
    }
    ...
    if (action === "update-class") { if (!isTeacherOrAdmin) return sendJson(res, 403, { error: "Forbidden" }); return sendJson(res, 200, { class: await classroomService.updateClass(auth, id, payload) }); }
    if (action === "delete-class") { if (!isTeacherOrAdmin) return sendJson(res, 403, { error: "Forbidden" }); await classroomService.deleteClass(auth, id); return sendJson(res, 200, { ok: true }); }
    if (action === "join-class") { if (!isStudent) return sendJson(res, 403, { error: "Forbidden" }); const classroom = await classroomService.joinClass(auth, String(payload.joinCode || "").trim().toUpperCase(), { id: `member-${cryptoRandom()}`, requestedAt: new Date().toISOString() }); return sendJson(res, 201, { class: classroom }); }
    if (action === "approve-membership") { if (!isTeacher) return sendJson(res, 403, { error: "Forbidden" }); await classroomService.approveMembership(auth, payload.membershipId); return sendJson(res, 200, { ok: true }); }
    if (action === "update-membership") { if (!isTeacher) return sendJson(res, 403, { error: "Forbidden" }); await classroomService.updateMembership(auth, id, payload.status); return sendJson(res, 200, { ok: true }); }
    if (action === "delete-membership") { await classroomService.deleteMembership(auth, id); return sendJson(res, 200, { ok: true }); }
    if (action === "create-user") { if (!isAdmin) return sendJson(res, 403, { error: "Forbidden" }); return sendJson(res, 201, { user: await createTenantUser(auth.tenant.id, payload) }); }
    if (action === "create-users-batch") { if (!isAdmin) return sendJson(res, 403, { error: "Forbidden" }); return sendJson(res, 201, await createTenantUsersBatch(auth.tenant.id, payload)); }
    if (action === "add-students-to-class") {
      ...
      const added = [], errors = [], users = await listTenantUsers(auth.tenant.id);
      for (const email of emails) {
        try {
          const normalized = String(email || "").trim().toLowerCase(); if (!normalized) throw new Error("Email kosong");
          const user = users.find((u) => u.email === normalized); if (!user) throw new Error("User tidak ditemukan");
    ...
    if (action === "create-students-batch") {
      ...
      for (const [index, u] of users.entries()) {
        try {
          const name = String(u.name || "").trim(); const email = String(u.email || "").trim().toLowerCase();
