...
function hideStreamPanel(ctx, panel) { panel?.classList.add("hidden"); }
function showQuestionStreamPlaceholder(ctx) { const { els } = ctx; els.aiStreamPlaceholder?.classList.remove("hidden"); if (els.aiStreamQuestions) els.aiStreamQuestions.innerHTML = ""; els.aiStreamPanel?.classList.remove("hidden"); els.aiStreamPanel?.classList.remove("ai-stream-done"); }
...
function finishQuestionStream(ctx) { const { els } = ctx; els.aiStreamPanel?.classList.add("ai-stream-done"); const first = els.aiStreamQuestions?.querySelector(".ai-stream-question"); if (first) { first.scrollIntoView({ behavior: "smooth", block: "center" }); first.classList.add("ai-stream-focus"); } }
