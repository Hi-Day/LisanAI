...
export function handleCreateManualAssessment(ctx) { const { els } = ctx; const config = readAssessmentForm(els); if (!config.classId) { showToast("Pilih kelas tujuan terlebih dahulu."); return; } ctx.pendingAssessmentConfig = config; const count = Math.max(1, Number(config.count) || 1); ctx.pendingQuestions = Array.from({ length: count }).map((_, i) => ({ id: `q-${i}`, prompt: "", focus: "", learningOutcomeIds: [], learningOutcomeId: "", outcome: "", rubric: "", ideal: "" })); renderQuestionEditor(ctx); goToWizardStep(ctx, 2); }
export function handleAddManualQuestion(ctx) { if (!ctx.pendingAssessmentConfig) { showToast("Buat atau buka penilaian dulu sebelum menambah soal."); return; } syncQuestionsFromEditor(ctx); const idx = ctx.pendingQuestions.length; ctx.pendingQuestions.push({ id: `q-${idx}`, prompt: "", focus: "", learningOutcomeIds: [], learningOutcomeId: "", outcome: "", rubric: "", ideal: "" }); renderQuestionEditor(ctx); }
...
export function syncQuestionsFromEditor(ctx) { const { els } = ctx; ctx.pendingQuestions = [...els.editableQuestionList.querySelectorAll(".editable-question")].map((item, index) => { const previous = ctx.pendingQuestions[index] || {}; const ids = Array.isArray(previous.learningOutcomeIds) ? previous.learningOutcomeIds : (previous.learningOutcomeId ? [previous.learningOutcomeId] : []); return { id: previous.id || `q-${index}`, prompt: item.querySelector("[data-field='prompt']").value.trim(), focus: item.querySelector("[data-field='focus']").value.trim(), learningOutcomeIds: ids, learningOutcomeId: ids[0] || "", outcome: item.querySelector("[data-field='outcome']").value.trim(), rubric: previous.rubric || "", ideal: item.querySelector("[data-field='ideal']").value.trim(), criteria: previous.criteria || [], probing: item.querySelector("[data-field='probing']")?.checked ?? !!previous.probing }; }); }
